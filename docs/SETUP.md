...
> ```
> Tìm dòng chứa URL dạng: `https://xxxxxx.trycloudflare.com` (ví dụ: `https://hart-represented-molecules-uncertainty.trycloudflare.com`). Dùng URL này làm địa chỉ HTTPS để kết nối Fanpage Facebook với n8n.
...
Tạo một workflow mới đặt tên là: **"Facebook Chatbot Connector"** trên n8n với 2 luồng Webhook độc lập:

#### Luồng 1: Xác Thực Webhook (Verify - GET)

```
[Webhook Node (GET)] → [Respond to Webhook (Verify hub.challenge)]
```

1. **Node 1: Webhook (Verify)**:
   - **HTTP Method**: Chọn **`GET`**
   - **Path**: Điền **`facebook-messenger`** (Link hoàn chỉnh: `https://<your-subdomain>.trycloudflare.com/webhook-test/facebook-messenger`)
   - **Respond**: Chọn **`Respond to Webhook`**
2. **Node 2: Respond to Webhook (Nối sau Node 1)**:
...
#### Luồng 2: Xử Lý Tin Nhắn Khách Hàng (Chat - POST)

```
[Webhook Node (POST)] 
    → [Code (Parse Messenger Body)]
    → [PostgreSQL (Lấy hoặc Tạo Chat Session)]
    → [AI Agent (Marina Chatbot)]
    → [HTTP Request (Gửi tin nhắn ngược lại cho khách)]
```

1. **Node 1: Webhook (Messages - POST)**:
   - **HTTP Method**: Chọn **`POST`**
   - **Path**: Điền giống hệt: **`facebook-messenger`**
   - **Respond**: Chọn **`Respond to Webhook`** (hoặc *When Last Node Finishes*)
   - **Options**: Bật **`Binary Data`** = `False`

2. **Node 2: Code (Parse Messenger Body - Nối sau Node 1)**:
...
3. **Node 3: PostgreSQL (Lấy hoặc tạo Session ID - Nối sau Node 2)**:
   - **Operation**: `Execute a SQL Query`
   - **Query**:
     ```sql
...
4. **Node 4: AI Agent (Marina - Nối sau Node 3)**:
...
5. **Node 5: HTTP Request (Gửi phản hồi về Facebook Messenger - Nối sau Node 4)**:
  - **Method**: `POST`
...
2. Tại phần **Webhooks** của Messenger:
   - **Callback URL**: Điền đường dẫn của bạn: `https://<your-subdomain>.trycloudflare.com/webhook/facebook-messenger` (Ví dụ: `https://hart-represented-molecules-uncertainty.trycloudflare.com/webhook/facebook-messenger`)
